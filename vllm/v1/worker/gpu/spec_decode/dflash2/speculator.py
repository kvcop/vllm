    ...
    for step in range(num_steps):
        ...
        if temperature == 0.0:
            ...
            if TRUNCATE:
                ...
                # candidates (the verify truncates the target the same way, so mass the
                # draft puts outside that support is a guaranteed rejection). The
                # truncated, renormalized q is what gets cached, so the verify stays
                # lossless. Rank-based: 16 candidates, all in registers.
                ...
                t_scores = tl.where(keep, t_scores, -float("inf"))
            ...
            sampled_scores = tl.where(mask, t_scores + noise, -float("inf"))
            best = tl.max(sampled_scores, axis=0)
            index = tl.min(tl.where(sampled_scores == best, offsets, BLOCK_K), axis=0)
            realized = t_scores

        # vLLM 0.27.1 caches temperature-APPLIED draft logits (its rejection sampler
        # reads them as-is); upstream main divides on load instead. With TRUNCATE the
        # cached row is the truncated q (-inf outside the kept support).
        tl.store(
...
class DFlash2Speculator(DFlashSpeculator):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # over by the model runner; VLLM_DFLASH2_DRAFT_TOPK_TOPP=0 disables the
        # truncation.
        ...
        #  NMIN/NSTRONG/AGREE   when to let the lookup replace a token the drafter
        # proposed
        ...
        # 12, not 32: the kernel picks the longest match and breaks ties by recency, and
        # a
        # longer cap makes it prefer an older long match over a newer short one. On
        # quote-and-explain work the newer one is the better predictor (3.21 vs 2.69
        # tokens
        ...
        # positions the drafter also proposed. Measured better at C1 (3.33 vs 3.27
        # tokens
        ...
        self._lookup_nmin_tail = int(
            os.environ.get("VLLM_DFLASH2_LOOKUP_NMIN_TAIL", "4")
        )
        self._lookup_long_min = int(os.environ.get("VLLM_DFLASH2_LOOKUP_LONGMIN", "6"))
        # Below this many tokens of context the long block is taken unconditionally, on
        # the
        ...
        # ships (4 request slots, 56k), because what the extra positions cost depends on
        # the
        # paged-attention layout as much as on the KV length.
        self._lookup_cheap_ctx = int(
            os.environ.get("VLLM_DFLASH2_LOOKUP_CHEAP_CTX", "0")
        )
        self._lookup_search = int(
            os.environ.get("VLLM_DFLASH2_LOOKUP_SEARCH", str(1 << 30))
        )
        ...
        # otherwise. The signal is the previous step's lookup decision, read from a
        # pinned
        # copy -- one step stale by construction, which costs at most one step at each
        # end
        ...
        # Entering the long block takes two qualifying steps in a row and leaving it
        # took
        # one, so a single step where the flag dropped out mid-copy cost three. It drops
        # out
        # for reasons that have nothing to do with the copy ending -- a line the lookup
        # cannot match, or a flag copy that had not landed yet -- and the same server
        # then
        # measured 13.76, 13.92, 13.92, 13.92 and 13.92 tokens per step on consecutive
        # runs
        ...
        #
        # Gating the hold on "the request is still emitting a full block a step" --
        # which
        # should distinguish a late flag from a finished copy -- removes the whole
        # effect
        # (13.92 again): by the time the flag drops the step it describes was not
        # saturated
        # either, so the two are not independent evidence.
        #
        # It only applies with one request in flight, and that restriction is not
        # caution.
        # This counter is one number for the whole batch, and unlike the entry condition
        # it
        # keeps the long block on through steps where the flags say no -- so with
        # several
        # requests, which block length a copying request gets starts to depend on when
        # the
        # others arrived. Different block length, different rounding, different greedy
        # text:
        # bench/labd_soak.py caught a verbatim copy coming out differently in two rounds
        # of
        ...
        if self._lookup:
            logger.info(
                ...
                self._lookup_nmin,
                self._lookup_nmax,
                self._lookup_nstrong,
                self._lookup_agree,
                self._lookup_nmin_tail,
                self._lookup_long_min,
                self._lookup_search,
            )

    def next_num_draft_tokens(self) -> int | None:
        """Return how many proposed tokens the scheduler should verify next.

        None means all proposed tokens.

        This runs on the host once per step, which is why the decision lives
        here and not in `_apply_lookup`: the draft pass is replayed from a
        captured CUDA graph, so Python runs at capture time only. The two inputs
        are the per-request flag the replayed fuse kernel writes (there is
        something to put in the tail) and what the finished step emitted. A
        long block costs step time on every request in the batch whether or not
        its tail is accepted, so it takes both, and unanimity across the batch.
        Below CHEAP_CTX tokens of context an extra verify position is nearly
        free (+6% per step at 1.5k against +27% at 25k), so the flag alone is
        enough there.
        """
        ...
        if self.draft_max_seq_len <= self._lookup_cheap_ctx:
            # An extra verify position costs attention proportional to the KV length:
            # +6%
        ...
        # Read the decision that landed from the previous step and start the copy for
        # the
        # next one. Reading it synchronously (`.item()`) is a device synchronise on
        # every
        # decode step and measured 5% -- more than the long block itself is worth on
        # most
        # work. One step of staleness costs a short step at the start of a copy run and
        # a
        # long one at its end.
        ...
        # middle of ordinary prose (a quoted phrase, a repeated list marker) and the
        # long
        # block it buys is then wasted. Waiting for the second one costs the first step
        # of
        ...
        if want and self._prev_want:
            ...
            # Coasting: re-entry costs two steps, so a run that has already earned the
            # long
    ...
    def _apply_lookup(self, num_reqs: int) -> None:
        """Fuse the drafted block with an earlier suffix's continuation.

        This fills verify positions past the drafter's own block. The draft
        distribution for every lookup-supplied position becomes a point mass on
        the proposed token, which keeps vLLM's rejection sampling exact.
        """
