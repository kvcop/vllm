...
class CudaGraphManager:
    ...
    def _init_candidates(self) -> None:
        ...
        if (
            ...
            # (v1/worker/gpu/spec_decode/dflash2/lookup.py fills the rest from the
            # request's
            ...
            if (
                ...
                _block = int(
                    (getattr(_cfg, "dflash_config", None) or {}).get("block_size", 0)
                )
                _short = _block + (
                ...
                if 0 < _short < self.decode_query_len:
                    ...
                    logger.info(
                        "Capturing decode graphs for query lengths %s "
                        "(the drafter's block and the full verify block).",
                        decode_query_lens,
                    )
