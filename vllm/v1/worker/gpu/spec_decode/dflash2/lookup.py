...
When the tokens the model just produced already occurred earlier in this
request's own context, the continuation is sitting right there: propose *that*
instead of what the model drafter guessed. One Triton program per request finds
the most recent occurrence of the longest suffix (NMIN..NMAX tokens) of the
token history and returns the tokens that followed it.

Three things make this worth more than a drafter override:

* the proposal is not capped by the drafter's block. The drafter emits the 7
  tokens it was trained for; a verbatim copy can be dozens of tokens long, and
  the lookup can fill every verify slot the server was launched with
  (`DFLASH_TOKENS`).
...
The proposal stays lossless: the caller rewrites the draft distribution for the
overridden positions to a point mass on the proposed token, which is a legal q
for vLLM's rejection sampler (accept probability becomes p(x), and the residual
(p - q)+ is computed from the same buffer), so the output distribution is
unchanged. Greedy requests never read q at all.

Cost is one pass over the request's history (an int32 UVA tensor vLLM already
maintains), with an NMIN-token reject test before any candidate is extended.
"""
...
# "longest match, then most recent" is a single max-reduction. max_model_len <
# SCORE_STRIDE.
...
def _suffix_lookup_kernel(
    ...
    for start in range(lo, hi, BLOCK):
        ...
        if tl.max(alive.to(tl.int32)) > 0:
            ...
            score = tl.where(
                alive, match.to(tl.int64) * SCORE_STRIDE + e.to(tl.int64), -1
            )
    ...
    # Tokens that actually follow the match inside the history. A match close to the
    # suffix
    # yields fewer than k of them; the caller leaves the rest of the block to the
    # drafter.
...
def _fuse_draft_kernel(
    ...
    Head (`idx < draft_block`): the drafter already proposed these, so taking
    the lookup's token *replaces* a considered guess and a coincidental match is
    a loss. A match of at least `nstrong` tokens is taken on its own; a shorter
    one (down to `nmin`) only if the drafter independently proposed the same
    first `agree_min` tokens -- two independent sources agreeing, one having
    looked at the hidden state and the other at the text.

    Tail (`idx >= draft_block`): nobody proposed these, so anything the match
    supplies is free and the threshold drops to `nmin_tail`. The only
    requirement is consistency with the head that precedes them -- either the
    lookup owns the head too, or the drafter independently agreed with it all
    the way to `draft_block`. Tail positions always get a point mass, since
    nothing else wrote a draft distribution for them.

    `take_flags` says only that this request has something to put in the tail: a
    match at least `long_min` long with enough tokens left to fill the block.
    Whether the long block is worth its step time is decided by the caller's
    controller, from what long blocks have actually returned (match length is a
    poor predictor: a 6-token match can carry fifteen correct tokens and a
    12-token one none).
    """
    req = tl.program_id(0)
    ...
    # draft_tokens is int64 (vllm/v1/worker/gpu/spec_decode/speculator.py); the history
    # the
    ...
    # Positions the match did not supply are not agreement: without this a match with
    # one
    ...
    # not the tail is accepted, so it takes evidence that a copy is *running*: the step
    # that
    # just finished accepted every token it was given (prev_acc = 1 + accepted, and a
    # short
    ...
    # --
    # measured, it needs a threshold of 22 before it stops losing on prose, by which
    # point
    # it has given up most of the win on copy work.
    # "This request has something to put in the tail." Whether the long block is worth
    # its
    # step time is decided by the caller: this kernel runs inside the draft model's
    # captured
...
def _point_mass_draft_logits_kernel(
    ...
    """Rewrite a sparse draft-logit row to a point mass on the proposed token.

    Every finite row entry stays listed in `cached_candidate_ids`, so the next
    step erases it. The token is read back from draft_tokens, so q always
    describes the proposal that is actually verified.
    """
...
def suffix_lookup(
    ...
    idx_mapping maps batch position -> request-state index, read with
    idx_mapping_stride. The DFlash speculators keep it as sample_idx_mapping,
    one entry per (request, step).
    """
...
def fuse_draft(
    ...
    draft_block is how many of the k verify positions the drafter itself
    proposed. The lookup fills the rest. It defaults to all k, meaning the
    drafter filled the block.
    """
